...
class Utility {
  ...
  static LatLng clusterPoint(
    ...
    double latOffset =
        distance * sin(angle) * (0.7 + random.nextDouble() * 0.6);
    double lonOffset =
        distance * cos(angle) * (0.7 + random.nextDouble() * 0.6);
  ...
  static LatLng randomWalk(LatLng position, int index) {
    ...
    double latChange =
        ...
        cos(
          angleRad,
        ); // Latitude change (using cosine for north/south movement)
    double lonChange =
        stepSize *
        sin(angleRad); // Longitude change (using sine for east/west movement)
