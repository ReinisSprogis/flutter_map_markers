...
class LatLongToOffsetDemoPage extends StatefulWidget {
  ...
  @override
  State<LatLongToOffsetDemoPage> createState() => _LatLongToOffsetDemoPageState();
}
...
class _LatLongToOffsetDemoPageState extends State<LatLongToOffsetDemoPage> {
  ...
  List<CanvasMarker> _generateMarkers(int count) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      final LatLng? previousPosition = generatedMarkers.isNotEmpty ? generatedMarkers.last.position : null;
      final LatLng markerPosition = Utility.randomWalk(previousPosition ?? london, i);
  ...
  void _drawArrowhead(Canvas canvas, Offset start, Offset end, Paint paint,Paint backgroundPaint) {
    ...
    final Offset arrowPoint1 =
        Offset(backwardDirection.dx * cosAngle - backwardDirection.dy * sinAngle, backwardDirection.dx * sinAngle + backwardDirection.dy * cosAngle) * arrowLength + end;
  ...
  CanvasMarker _generateMarker(LatLng position, LatLng? previousPosition, Color color) {
    ...
    return CanvasMarker(
      ...
      painter: (canvas, center, metersToPixels, latLngToPixelOffset, zoomLevel) {
        ...
        final Offset offsetPosition = latLngToPixelOffset(previousPosition ?? london);
    ...
    return Scaffold(
      ...
      body: FlutterMap(
        ...
        children: [
          ...
          CanvasMarkerLayer(markers: markers, drawHitMarkerLast: true, cullMarkers: false),
        ],
      ),
